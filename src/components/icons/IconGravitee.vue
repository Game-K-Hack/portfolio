<template>
    <svg :width="size" :height="size" viewBox="0 0 917 876" fill="none" xmlns="http://www.w3.org/2000/svg" :aria-labelledby="uid" role="img" class="transition-colors duration-300" :class="{'rounded-[15%]': $options.metadata.rounded}">
        <title :id="uid">{{ $t("skills.items.gravitee.name") }}</title>
        ...
        <defs>
            <linearGradient :id="`${uid}-gradient`" x1="579.881" y1="28" x2="211.381" y2="796.5" gradientUnits="userSpaceOnUse">
                <stop :stop-color="themeClasses.c1" />
                <stop offset="1" :stop-color="themeClasses.c2" />
            </linearGradient>
        </defs>
    </svg>
</template>
